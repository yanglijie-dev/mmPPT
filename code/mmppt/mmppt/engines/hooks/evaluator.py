...
import mmppt.utils.comm as comm
from mmppt.utils.misc import intersection_and_union_gpu
...
@HOOKS.register_module()
class mmBodyEvaluator(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.eval()
    ...
    def eval(self):
        self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
        self.trainer.model.eval()
        self.trainer.storage.reset_histories()#清空storage里存储的历时数据记录
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.no_grad():
                output_dict = self.trainer.model(input_dict,self.trainer.epoch)
            #loss = output_dict["loss"]
            self.trainer.comm_info["model_output_dict"] = {key: value.detach() for key, value in output_dict[0].items()}# model_output_dict的值决定了after_epoch里往日志里记录的内容

            if "model_output_dict" in self.trainer.comm_info.keys():
                model_output_dict = self.trainer.comm_info["model_output_dict"]
                self.model_output_keys = model_output_dict.keys()
                for key in self.model_output_keys:
                    self.trainer.comm_info["iter_info"] += "{key}: {value:.4f} ".format(key=key, value=model_output_dict[key].item())
                    self.trainer.storage.put_scalar(key, model_output_dict[key].item())#storage保存历史值，方便后续计算平均值

            lr = self.trainer.optimizer.state_dict()["param_groups"][0]["lr"]
            self.trainer.comm_info["iter_info"] += "Lr: {lr:.5f}".format(lr=lr)
            self.trainer.logger.info(
                ("Test: [{iter}/{max_iter}] ".format(iter=i + 1, max_iter=len(self.trainer.val_loader))) + "Batch " +
                self.trainer.comm_info["iter_info"])
            self.trainer.comm_info["iter_info"] = ""

            if self.trainer.writer is not None:
                self.trainer.writer.add_scalar("lr", lr)
                for key in self.model_output_keys:
        ...
        #    )
        #打印batch loss的平均值
        #self.trainer.comm_info["iter_info"] = "Evaluation loss: " + "{value:.4f}".format(value=self.trainer.storage.history("loss").avg)
        epoch_info = "Epoch "+ str(self.trainer.epoch)+" Evaluation result: "
        for key in self.model_output_keys:#记录当前epoch里的总体损失值
            epoch_info += "{key}: {value:.4f} ".format(
                key=key, value=self.trainer.storage.history(key).avg    #storage保存历史值，方便后续计算平均值
            )
        self.trainer.logger.info(epoch_info)#在console里输出
        '''
        # 在console输出evaluation的bath loss
        self.trainer.comm_info["losses"] = self.trainer.model.criteria.losses
        self.trainer.comm_info["iter_info"] = "Evaluation result: Batch loss: "
        for batch_loss in self.trainer.comm_info["losses"].batch_loss:
            self.trainer.comm_info["iter_info"] += "{value:.4f} ".format(value= batch_loss.item())
        self.trainer.logger.info(self.trainer.comm_info["iter_info"])

        # 在console输出evaluation的不同loss分量
        self.trainer.comm_info["iter_info"] = "Evaluation result: Loss components: "
        for key in self.trainer.comm_info["losses"].loss_dict:
            self.trainer.comm_info["iter_info"] += key + ": "
            for value in self.trainer.comm_info["losses"].loss_dict[key]:
                self.trainer.comm_info["iter_info"] += "{value:.4f}".format(value=value.item()) + ", "
        self.trainer.logger.info(self.trainer.comm_info["iter_info"])
        '''

        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")

        self.trainer.comm_info["current_metric_value"] = self.trainer.storage.history("loss").avg  # 用于后续判断当前epoch得到的pth是否为model_best.pth
