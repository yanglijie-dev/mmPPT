...
from mmppt.utils.comm import is_main_process, synchronize, get_world_size
from mmppt.utils.cache import shared_dict
import mmppt.utils.comm as comm
from mmppt.engines.test import TESTERS
...
class InformationWriter(HookBase):
    ...
    def after_epoch(self):
        epoch_info = "Epoch "+ str(self.trainer.epoch)+" train result: "
        for key in self.model_output_keys:
            epoch_info += "{key}: {value:.4f} ".format(
                key=key, value=self.trainer.storage.history(key).avg
            )
        for key in self.trainer.model.criteria.losses.loss_dict:
            epoch_info += "{key}:".format(key=key)
            for i in range(len(self.trainer.model.criteria.losses.loss_dict[key])):
                value = self.trainer.model.criteria.losses.loss_dict[key][i].item()
                epoch_info += "{value:.4f}, ".format(value=value)
        #epoch_info +="\n"
...
@HOOKS.register_module()
class mmBodyInformationWriter(HookBase):
    def __init__(self):
        self.curr_iter = 0
        self.model_output_keys = []
    ...
    def before_epoch(self):
        ...
        self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Training >>>>>>>>>>>>>>>>")
    ...
    def after_step(self):
        ...
        for key in self.model_output_keys:
            if key =='loss':
                self.trainer.comm_info["iter_info"] += "{key}: {value:.4f} ".format(
                    key="Batch loss", value=self.trainer.storage.history(key).val
                )
            else:
                self.trainer.comm_info["iter_info"] += "{key}: {value:.4f} ".format(
                    key=key, value=self.trainer.storage.history(key).val
    ...
    def after_epoch(self):
        ...
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Training <<<<<<<<<<<<<<<<<")
...
@HOOKS.register_module()
class mmBodyCheckpointSaver(HookBase):
    def __init__(self, save_freq=None):
        self.save_freq = save_freq  # None or int, None indicate only save model last
...
class PreciseEvaluator(HookBase):
    ...
    def after_train(self):
        ...
        if self.trainer.cfg.test_only == True:
            os.system(f'cp {os.path.join(self.trainer.cfg.test_only_pth_path, "model", "model_best.pth")} {os.path.join(self.trainer.cfg.save_path, "model", "model_best.pth")}')
            os.system(f'cp {os.path.join(self.trainer.cfg.test_only_pth_path, "model", "model_last.pth")} {os.path.join(self.trainer.cfg.save_path, "model", "model_last.pth")}')

        if self.test_last:
            self.trainer.logger.info("=> Testing on model_last.pth")
        else:
            self.trainer.logger.info("=> Testing on model_best.pth")
...
class DataCacheOperator(HookBase):
    ...
    def get_cache_name(self, data_path):
        ...
        return "mmppt" + data_name.replace(os.path.sep, "-")
    ...
    def before_train(self):
        ...
        if is_main_process():
            ...
            for i in range(len(dataset)):
                data_dict = dataset[i]
                name = data_dict["name"]
                shared_dict(f"mmppt-{name}", data_dict)
        synchronize()
