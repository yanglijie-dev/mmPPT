...
from mmppt.datasets import build_dataset, collate_fn
from mmppt.models import build_model
from mmppt.utils.logger import get_root_logger
from mmppt.utils.registry import Registry
from mmppt.utils.misc import (
    AverageMeter,
...
@TESTERS.register_module()
class PointCloudTester(TesterBase):
    def test(self):
        logger = get_root_logger()
        logger.info(">>>>>>>>>>>>> Start Test on scene {value} >>>>>>>>>>>>>".format(value=self.cfg.data.test.test_scene))
        #batch_time = AverageMeter()
        #intersection_meter = AverageMeter()
        #union_meter = AverageMeter()
        # target_meter = AverageMeter()
        ...
        for i, input_dict in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                output_dict, smpl_vertices = self.model(input_dict, self.cfg._cfg_dict['epoch'])#output_dict保存模型的损失，smpl_vertices记录当前模型输出的smpl数据，可以用来生成smpl模型

            self.smpl_save( smpl_vertices, input_dict['file_id'])# 保存模型输出的smpl参数，用于展现smpl效果

            Meter = {key: AverageMeter() for key in output_dict} if len(Meter)==0 else Meter

            for key in output_dict:
                Meter[key].update(output_dict[key])

            logger.info(
                "Test: [{}/{}] "
                "Batch loss: {Loss:.4f} "
                "trans_Loss: {trans_loss:.4f} "
                "pose_Loss: {pose_loss:.4f} "
                "shape_Loss: {shape_loss:.4f} "
                "joints_Loss: {joints_loss:.4f} "
                "vertices_Loss: {vertices_loss:.4f} ".format(
                    i+1,
                    len(self.test_loader),
                    Loss=output_dict['loss'],
                    trans_loss=output_dict['trans_loss'],
                    pose_loss=output_dict['pose_loss'],
                    shape_loss=output_dict['shape_loss'],
                    joints_loss=output_dict['joints_loss'],
                    vertices_loss=output_dict['vertices_loss'],
                )
            )
        logger.info(
            "Test result: "
            "Batch loss: {Loss.avg:.4f} "
            "trans_Loss: {trans_loss.avg:.4f} "
            "pose_Loss: {pose_loss.avg:.4f} "
            "shape_Loss: {shape_loss.avg:.4f} "
            "joints_Loss: {joints_loss.avg:.4f} "
            "vertices_Loss: {vertices_loss.avg:.4f} ".format(
                Loss=Meter['loss'],
                trans_loss=Meter['trans_loss'],
                pose_loss=Meter['pose_loss'],
                shape_loss=Meter['shape_loss'],
                joints_loss=Meter['joints_loss'],
                vertices_loss=Meter['vertices_loss'],
            )
        )


        logger.info(">>>>>>>>>>>>> End Test on scene {value} >>>>>>>>>>>>>".format(value=self.cfg.data.test.test_scene))

    @staticmethod
    def collate_fn(batch):
        return collate_fn(batch)

    def smpl_save(self,save_data, file_id): #保存模型输出的smpl参数，用于展现smpl效果
        if self.cfg.smpl_vertices_store:
            # folder_path = (self.cfg.save_path + "/smpl_data/")
            folder_path = (self.cfg['test_only_pth_path'] + "/test_result/" +self.cfg['test_scene'][0] )
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            for idx in range(self.cfg.batch_size):
                container = {}
                container['output_vert'] = save_data['output_verts'][idx,:,:].cpu().detach().numpy()
                container['target_vert'] = save_data['target_verts'][idx,:,:].cpu().detach().numpy()
                file_path = folder_path +"/" + file_id[idx] + ".pickle"
