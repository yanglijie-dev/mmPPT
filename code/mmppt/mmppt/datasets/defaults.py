...
class DefaultDataset(Dataset):
    ...
    def __init__(
        ...
        self.data_list = self.get_data_list()#加载所有数据的元信息列表
    ...
    def prepare_train_data(self, idx):#根据数据索引idx加载lidar点云，并实现数据增强，以及得到grid_coord
        # load data
        data_dict = self.get_data(idx)#根据数据索引idx加载lidar点云
        data_dict = self.transform(data_dict)#对点云进行缩放、旋转、扭曲等操作，实现数据增强，并得到grid_coord
        return data_dict
