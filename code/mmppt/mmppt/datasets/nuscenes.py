...
class NuScenesDataset(DefaultDataset):
    ...
    def get_data_list(self):#加载所有数据的元信息列表
        ...
        for info_path in info_paths:
            with open(info_path, "rb") as f:
                info = pickle.load(f)#包含train或val的点云数据信息。
                data_list.extend(info)
        return data_list

    def get_data(self, idx):
        #print("curr idx=",idx)
        data = self.data_list[idx % len(self.data_list)]#从data_list里加载索引为idx的数据
        lidar_path = os.path.join(self.data_root, "raw", data["lidar_path"])#从data里得得到单个lidar bin数据的路径
        points = np.fromfile(str(lidar_path), dtype=np.float32, count=-1).reshape(
            [-1, 5]
        )#读取lidar_path的lidar点云
        coord = points[:, :3]#points前三维是坐标
        strength = points[:, 3].reshape([-1, 1]) / 255  #points第4维是强度# scale strength to [0, 1]

        if "gt_segment_path" in data.keys():#获取每个点云的ground truth
            gt_segment_path = os.path.join(
                self.data_root, "raw", data["gt_segment_path"]
            )
            segment = np.fromfile(
                str(gt_segment_path), dtype=np.uint8, count=-1  #count=-1 表示读取整个文件的内容
            ).reshape([-1])
            segment = np.vectorize(self.learning_map.__getitem__)(segment).astype(#使用vectorize函数将self.learning_map.__getitem__方法向量化，这意味着它可以接受一维数组作为输入，并返回对应元素经过映射后的新数组
                np.int64
            )
        else:
    ...
    @staticmethod
    def get_learning_map(ignore_index):#定义一个静态方法来生成学习映射，用于将原始标签映射到新的标签。ignore_index代表忽略该label
        learning_map = {
            0: ignore_index,
            1: ignore_index,
            2: 6,
