...
class NormalizeCoord(object):
    def __call__(self, data_dict):
        if "coord" in data_dict.keys():
            ...
            m = np.max(np.abs(data_dict["coord"] ), axis=0)
...
@TRANSFORMS.register_module()
class GridSample(object):#对点云进行grid sample得到grid_coord，同时降低点云数目
    def __init__(
        self,
        grid_size=0.05,
        hash_type="fnv",
        ...
        self.use_all_grid_points = use_all_grid_points
    def __call__(self, data_dict):
        ...
        if self.use_all_grid_points:#todo:这里相当于直接跳过了grid_sample的大部分功能，是否可以跳过还需要思考下
            data_dict['grid_coord'] = grid_coord
            return data_dict
        scaled_coord -= min_coord
        min_coord = min_coord * np.array(self.grid_size)
        key = self.hash(grid_coord)#使用之前定义的哈希函数对网格坐标进行哈希处理，生成一维的键
        idx_sort = np.argsort(key)#对哈希后的键进行排序，并记录排序前的索引位置
        key_sort = key[idx_sort]#排序后的键
        _, inverse, count = np.unique(key_sort, return_inverse=True, return_counts=True)#inverse:从0开始，为每一个键赋予一个值；count:每个键的数量
...
class Compose(object):
    ...
    def __call__(self, data_dict):
        for t in self.transforms:
            data_dict = t(data_dict)#对data_dict进行各种数据增强的操作,并得到grid_coord
        return data_dict
